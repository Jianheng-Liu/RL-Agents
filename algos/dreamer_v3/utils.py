...
def prepare_obs(
    obs: Dict[str, np.ndarray], *, cnn_keys: Sequence[str] = [], num_envs: int = 1, **kwargs
) -> Dict[str, Tensor]:
    torch_obs = {}
    for k, v in obs.items():
        torch_obs[k] = torch.from_numpy(v.copy()).to(torch.device("cuda" if torch.cuda.is_available() else "cpu")).float()
        if k in cnn_keys:
...
def log_models_from_checkpoint(
    env: gym.Env | gym.Wrapper, cfg: Dict[str, Any], state: Dict[str, Any]
) -> Sequence["ModelInfo"]:
    ...
    with mlflow.start_run(run_id=cfg.run.id, experiment_id=cfg.experiment.id, run_name=cfg.run.name, nested=True) as _:
        model_info["world_model"] = mlflow.pytorch.log_model(world_model, artifact_path="world_model")
        model_info["actor"] = mlflow.pytorch.log_model(actor, artifact_path="actor")
        model_info["critic"] = mlflow.pytorch.log_model(critic, artifact_path="critic")
