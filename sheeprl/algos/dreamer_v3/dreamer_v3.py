...
def train(
    ...
    Args:
        world_model (WorldModel): the world model.
        actor (torch.nn.Module): the actor model.
    ...
    if cfg.algo.world_model.clip_gradients is not None and cfg.algo.world_model.clip_gradients > 0:
        world_model_grads = torch.nn.utils.clip_grad_norm_(
            world_model.parameters(), cfg.algo.world_model.clip_gradients
        )
    ...
    if cfg.algo.actor.clip_gradients is not None and cfg.algo.actor.clip_gradients > 0:
        actor_grads = torch.nn.utils.clip_grad_norm_(actor.parameters(), max_norm=cfg.algo.actor.clip_gradients)
    ...
    if cfg.algo.critic.clip_gradients is not None and cfg.algo.critic.clip_gradients > 0:
        critic_grads = torch.nn.utils.clip_grad_norm_(critic.parameters(), max_norm=cfg.algo.critic.clip_gradients)
...
def main(cfg: Dict[str, Any]):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    rank = 0  # Assume single process
    world_size = 1  # Assume single process

    state = None
    if cfg.checkpoint.resume_from:
        state = torch.load(cfg.checkpoint.resume_from, map_location=device)
